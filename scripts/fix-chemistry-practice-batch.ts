...
function transformChemistryPracticeSource(
  sourceText: string,
  filePath: string,
): {
  text: string;
...
function main(): void {
  const names = fs.readdirSync(PRACTICE_DIR).filter((n) => /^chemistry-topic\d+-practice\.ts$/.test(n));
  names.sort();
  ...
  for (const name of names) {
    ...
    const { text: after, mcqFixed, workedFixed } = transformChemistryPracticeSource(before, fp);
    if (after !== before) {
      filesChanged += 1;
      totalMcq += mcqFixed;
